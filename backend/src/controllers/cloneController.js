...
exports.generateCode = async (req, res) => {
    ...
    if (!workspaceId) {
        return res.status(400).json({ message: 'Anda harus memiliki workspace untuk membuat undangan.' });
    }
...
exports.useCode = async (req, res) => {
    ...
    if (!code) return res.status(400).json({ message: 'Kode tidak boleh kosong' });
    try {
