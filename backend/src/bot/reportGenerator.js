...
async function generateAndSendDailyReports() {
    ...
    } catch (error) {const pool = require('../config/database');
const { sendWhatsAppMessage } = require('../services/whatsappService');
...
async function generateSingleReport(workspace) {
    ...
    try {
        ...
        await client.connect();
        const safeWriteAndCount = async (command, params = []) => {
            try {
        ...
        const [pppoeTotal, pppoeActive, hotspotTotal, hotspotActive] = await Promise.all([
            safeWriteAndCount('/ppp/secret/print', ['?service=pppoe']),
            safeWriteAndCount('/ppp/active/print', ['?service=pppoe']),
            safeWriteAndCount('/ip/hotspot/user/print'),
            safeWriteAndCount('/ip/hotspot/active/print')
        ]);
        client.close();
